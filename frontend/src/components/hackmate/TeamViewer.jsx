...
const TeamViewer = () => {
  ...
  const handleJoinRequest = async () => {
    ...
    try {
      ...
      toast.success(response.data.message || "You have successfully joined the team.");
    } catch (error) {
      console.error("Error sending join request:", error);
      toast.error(
        error.response?.data.message || "Failed to join the team. Please try again."
      );
    } finally {
  ...
  const handleDeclineRequest = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/hackmate/declineRequest/${teamId}/${userId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success(response.data.message);
      // Optionally refresh team data to update UI
    } catch (error) {
  ...
  const handleAcceptRequest = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/hackmate/acceptRequest/${teamId}/${userId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success(response.data.message);
  ...
  const handleRemoveMember = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/hackmate/removeMember/${teamId}/${userId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success(response.data.message);
      ...
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center gap-3">
          <div className="bg-indigo-500 text-white rounded-full w-10 h-10 flex items-center justify-center font-bold">
            {/* Optionally, add initials or an icon here */}
          </div>
  ...
  <p className="text-gray-400 font-bold">You must be a member to view this team's members.</p>
)}
          ...
          {adminId === userId || members.some((member) => member.userId === userId) ? (
  null
) : (
  <div className="mt-6">
    <button
