...
const ResourcesDetail = () => {
  ...
  const handleCategoryClick = (e) => {
    setActiveCategory(e.target.innerHTML); // Store the clicked category
  };
        ...
        <div className="my-8">
          <nav>
            <ul className="flex text-white gap-10 font-semibold">
              {["Books", "Notes", "Assignment", "Practical", "Papers", "Playlist"].map(
                (category, index) => (
                  <li
                    key={index}
                    className={`cursor-pointer ${
                      activeCategory === category ? "text-purple-500 font-bold" : ""
                    }`}
...
const ResourceDetailsCategory = ({ resourceId, data, activeCategory }) => {
  ...
  if (!resourceData) {
    return <div className="text-white">No data available for {activeCategory}.</div>;
  }

  return (
    <div className="my-5">
      {Object.keys(resourceData).map((semester, index) => (
        <div key={index} className="my-5">
          <h3 className="text-2xl font-semibold text-white mb-4">{semester}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-20">
            {Object.keys(resourceData[semester]).map((subject, idx) => (
              <div
                key={idx}
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {resourceData[semester][subject].Units.map((unit, unitIdx) => (
                    <button
                      key={unitIdx}
